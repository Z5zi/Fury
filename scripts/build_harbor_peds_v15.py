#!/usr/bin/env python3
"""Harbor Metro Cycle-15 AAA pedestrians — ALL Antonia.Polygon (CC0).

All five Antonia.Polygon CC0 in NATURAL idle with arms DOWN (near-vertical; not T / wide-A).
Male silhouette via scale + short hair. Fitted body-derived garments (no rectangular clothing shells).
...
PEDS = [
    ...
    dict(name="ivy", height=1.66, pose="idle_e", jacket=True, male=False,
         skin=(0.88, 0.72, 0.62, 1.0), shirt=(0.18, 0.28, 0.22, 1.0),
...
def apply_natural_apose(body, pose_name="idle_a"):
    """RIGID T→idle drop: whole arm rotates as a unit (no banana falloff)."""
    import bmesh
    from collections import deque

    angle_table = {
        "idle_a": 95, "idle_b": 92, "idle_c": 98, "idle_d": 94, "idle_e": 96,
    }
    asym = {
        "idle_a": (0, 4), "idle_b": (5, -3), "idle_c": (-3, 6),
        "idle_d": (4, 2), "idle_e": (6, -4),
    }
    elbow_deg = {
        "idle_a": 14, "idle_b": 20, "idle_c": 10, "idle_d": 16, "idle_e": 12,
    }
    ...
    for sign, extra in ((1, dL), (-1, dR)):
        ...
        while q:
            ...
            for n in adj[i]:
                if n in visited:
                    continue
                nv = bm.verts[n]
                # Include full arm down to mid-ribcage side; stop at torso core
        ...
        # RIGID: every arm vert gets full angle (straight limb, not banana)
        for i in visited:
            v = bm.verts[i]
            v.co = ry(Vector(v.co), sh, ang)
        # Mild elbow bend on lower half of arm (verts now lower after drop)
        for i in visited:
            v = bm.verts[i]
            if v.co.z > 0.58 * h:
                continue
            # elbow pivot roughly mid-arm
    ...
    print("APOSE", pose_name, "W", round(body.dimensions.x, 3), "verts_arm", "ok")
...
def add_fabric_clothes(body, mats, spec):
    """Body-painted wardrobe + shoes + rear hair. NO torso shells / capsules / boxes."""
    style_by_name = {
        "rae": "blazer", "dane": "hoodie", "suki": "tee",
        "noah": "jacket", "ivy": "coat",
    }
    style = style_by_name.get(spec["name"], "tee")
    male = spec.get("male", False)
    paint_body_clothing(body, mats, {**spec, "jacket": style in ("blazer", "jacket", "coat", "hoodie")})
    ...
    # Shoes — small rounded capsules at feet only
    for sx in (-0.075 if male else -0.06, 0.075 if male else 0.06):
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=16, ring_count=10, radius=1.0,
            location=(cx + sx, cy + 0.06, 0.035))
        sh = bpy.context.active_object
        sh.scale = (0.055, 0.11, 0.035)
        bpy.ops.object.transform_apply(scale=True)
        apply_mat(sh, mats["Shoes"])
        shade_smooth(sh)
        objs.append(sh)

    # Hair — sit on scalp BACK, not over eyes
    if male:
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=24, ring_count=14, radius=1.0,
            location=(cx, cy - 0.01, h * 0.955))
        hair = bpy.context.active_object
        hair.scale = (0.10, 0.095, 0.045)
        bpy.ops.object.transform_apply(scale=True)
        apply_mat(hair, mats["Hair"])
        shade_smooth(hair)
        objs.append(hair)
    else:
        # Crown cap (high, slightly back)
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=24, ring_count=14, radius=1.0,
            location=(cx, cy - 0.02, h * 0.958))
        hair = bpy.context.active_object
        hair.scale = (0.105, 0.10, 0.055)
        bpy.ops.object.transform_apply(scale=True)
        apply_mat(hair, mats["Hair"])
        shade_smooth(hair)
        objs.append(hair)
        # Rear bob volume only (behind head)
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=20, ring_count=12, radius=1.0,
            location=(cx, cy - 0.08, h * 0.90))
        bob = bpy.context.active_object
        bob.scale = (0.09, 0.06, 0.08)
        bpy.ops.object.transform_apply(scale=True)
        apply_mat(bob, mats["Hair"])
        shade_smooth(bob)
        objs.append(bob)
    return objs
