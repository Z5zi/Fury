#!/usr/bin/env python3
"""Cycle-15 PRIMARY ChatGPT evidence: Blender Cycles beauty — Meridian Mutual block.

Hard fails vs C14 (2.8 NO-SHIP):
...
def hide_imported_grounds():
    for o in list(bpy.data.objects):
        ...
        if any(k in n for k in ("sidewalk", "asphalt", "road", "ground", "plaza", "pavement", "curb")):
            if "wetasphalt" in n or "c14_" in n or "c15_" in n or "authored" in n:
                continue
            o.hide_render = True
            o.hide_viewport = True
        dims = o.dimensions
...
def make_wet_asphalt_ph():
    """DARK aggregate asphalt FIRST. Wet only as sparse shallow puddle islands.
    Dry: high roughness. Clearcoat ONLY inside puddles. NO pale chrome sheets.
    """
    bpy.ops.mesh.primitive_plane_add(size=1, location=(18, -12, 0.012))
    ground = bpy.context.active_object
    ground.name = "C15_RoadAsphalt"
    ...
    mat = bpy.data.materials.new("C15_RoadAsphalt")
    mat.use_nodes = True
    nt = mat.node_tree
    nodes, links = nt.nodes, nt.links
    nodes.clear()
    ...
    bsdf = nodes.new("ShaderNodeBsdfPrincipled")
    bsdf.inputs["Metallic"].default_value = 0.0
    # Keep dry asphalt from reading as chrome
    ...
    tex = nodes.new("ShaderNodeTexCoord")
    mapping = nodes.new("ShaderNodeMapping")
    # Dense tile so aggregate grain reads at street scale
    mapping.inputs["Scale"].default_value = (36.0, 28.0, 1.0)
    ...
    # Preserve aggregate: mild darken only (NOT crush to black clay)
    darken = nodes.new("ShaderNodeMix")
    darken.data_type = "RGBA"
    darken.inputs["Factor"].default_value = 0.42
    dark_col = nodes.new("ShaderNodeRGB")
    dark_col.outputs[0].default_value = (0.035, 0.035, 0.038, 1)
    links.new(diff_n.outputs["Color"], darken.inputs["A"])
    links.new(dark_col.outputs[0], darken.inputs["B"])
    # Broad-scale breakup so it does not read as flat grey
    col_noise = nodes.new("ShaderNodeTexNoise")
    col_noise.inputs["Scale"].default_value = 1.4
    ...
    # --- Sparse puddle islands (soft mask, ~10% coverage) ---
    puddle_map = nodes.new("ShaderNodeMapping")
    puddle_map.inputs["Scale"].default_value = (1.1, 0.85, 1.0)
    links.new(tex.outputs["Object"], puddle_map.inputs["Vector"])
    n1 = nodes.new("ShaderNodeTexNoise")
    n1.inputs["Scale"].default_value = 2.8
    n1.inputs["Detail"].default_value = 8.0
    n1.inputs["Roughness"].default_value = 0.65
    links.new(puddle_map.outputs["Vector"], n1.inputs["Vector"])
    n2 = nodes.new("ShaderNodeTexNoise")
    n2.inputs["Scale"].default_value = 7.5
    n2.inputs["Detail"].default_value = 4.0
    links.new(puddle_map.outputs["Vector"], n2.inputs["Vector"])
    mul = nodes.new("ShaderNodeMath")
    mul.operation = "MULTIPLY"
    links.new(n1.outputs["Fac"], mul.inputs[0])
    links.new(n2.outputs["Fac"], mul.inputs[1])
    # Soft threshold via colorramp (NOT binary LESS_THAN sheets)
    ...
    links.new(mul.outputs[0], ramp.inputs["Fac"])
    # Invert so only HIGH noise peaks = wet (sparse islands)
    # Actually: high mul = wet. Good.
    ...
    if nor:
        ...
        nrm = nodes.new("ShaderNodeNormalMap")
        nrm.inputs["Strength"].default_value = 1.55
    ...
    # Sidewalk — concrete, matte
    bpy.ops.mesh.primitive_cube_add(size=1, location=(18, 4.5, 0.08))
    walk = bpy.context.active_object
    walk.scale = (70, 6.0, 0.12)
    bpy.ops.object.transform_apply(scale=True)
    walk.name = "C15_Sidewalk"
    ensure_uv(walk)
    wmat = make_ph_mat("C15_Sidewalk", "concrete_wall_008", scale=6.0, roughness_boost=0.18)
    walk.data.materials.append(wmat)

    # Lane markings (matte paint)
    for i, x in enumerate((6.0, 16.0, 26.0, 36.0)):
        ...
        bpy.ops.object.transform_apply(scale=True)
        mark.name = f"C15_Lane_{i}"
        mm = bpy.data.materials.new(f"C15_Lane_{i}")
        mm.use_nodes = True
    ...
    # NO geometric chrome puddle decals — wet lives only in the shader mask.
    print("C15_ASPHALT_BUILT dark-aggregate + sparse soft puddles")
...
def force_architecture_ph():
    ...
    for o in bpy.data.objects:
        if o.type != "MESH":
            continue
        n = o.name.lower()
        if any(k in n for k in ("c14_wet", "c15_road", "c14_puddle", "c14_lane", "c15_lane", "c14_sidewalk", "c15_sidewalk",
...
def build_lobby_interior():
    ...
    brass = make_metal_mat("C15_LobbyBrass", (0.45, 0.32, 0.12, 1), 0.28)
    leather = bpy.data.materials.new("C15_LobbyLeather")
    ...
    for i, x in enumerate((ox - 3.2, ox - 1.6, ox + 1.6, ox + 3.2)):
        ...
        for sx in (-0.18, 0.18):
            for sy in (-0.18, 0.18):
                ...
                leg.name = f"C15_WaitLeg_{i}_{sx}_{sy}"
    ...
    bpy.ops.object.transform_apply(scale=True)
    kb.name = "C15_Keyboard"
    kb.data.materials.append(make_metal_mat("C15_KB", (0.05, 0.05, 0.06, 1), 0.35))
    ...
    for i, x in enumerate((ox - 2.5, ox, ox + 2.5)):
        ...
        bpy.ops.object.transform_apply(scale=True)
        beam.name = f"C15_Coffer_{i}"
        beam.data.materials.append(make_ph_mat(f"C15_CofferMat_{i}", "plastered_wall_02", scale=1.5))
...
def force_ped_mats():
    """Keep wardrobe readable; kill peach Invisible and blaze-orange shirts."""
    for mat in bpy.data.materials:
        bsdf = get_bsdf(mat)
        if not bsdf:
            continue
...
def place_peds():
    """Five distinct idle silhouettes — Antonia v15 fitted garments, arms down."""
    specs = [
        ("hm_ped_rae.obj", (6.6, 0.0, 13.8), -1.75),
        ("hm_ped_dane.obj", (7.9, 0.0, 14.9), -2.05),
        ("hm_ped_suki.obj", (9.2, 0.0, 13.5), -1.55),
        ("hm_ped_noah.obj", (8.5, 0.0, 13.1), -1.95),
        ("hm_ped_ivy.obj", (10.1, 0.0, 14.4), -1.40),
    ]
    for fn, pos, yaw in specs:
        v15 = PEDS / fn.replace(".obj", "_v15.obj")
        path = v15 if v15.exists() else (PEDS / fn)
        if not path.exists():
            path = PEDS / fn
        objs = import_obj(path)
        loc = fury_to_b(*pos)
...
def add_authored_facade_detail():
    ...
    frame_m = make_metal_mat("C15_WinFrame", (0.06, 0.06, 0.07, 1), 0.42)
    sill_m = make_ph_mat("C15_Sill", "concrete_wall_008", scale=8.0, darken=0.1)
    ...
    for i, x in enumerate((10.0, 14.0, 22.0)):
        ...
        for sx in (-0.55, 0.55):
            ...
            bpy.ops.object.transform_apply(scale=True)
            leg.name = f"C15_BenchLeg_{i}_{sx}"
    ...
    for i, x in enumerate((12.0, 16.0, 26.0, 30.0)):
        ...
        pl.name = f"C15_Planter_{i}"
        pl.data.materials.append(make_ph_mat(f"C15_PlanterMat_{i}", "concrete_wall_008", scale=3.0))
        bpy.ops.mesh.primitive_uv_sphere_add(radius=0.32, location=(x, 6.0, 0.7))
        bush = bpy.context.active_object
        bush.scale = (1.0, 1.0, 0.7)
        bpy.ops.object.transform_apply(scale=True)
        bush.name = f"C15_Bush_{i}"
        bm = bpy.data.materials.new(f"C15_BushMat_{i}")
        bm.use_nodes = True
    ...
    hy.name = "C15_Hydrant"
    hm = bpy.data.materials.new("C15_HydrantMat")
    hm.use_nodes = True
