#!/usr/bin/env python3
"""Cycle-12 PRIMARY ChatGPT evidence: Blender Cycles beauty — Meridian Mutual block.
...
def hide_junk():
    for o in list(bpy.data.objects):
        ...
        if any(k in n for k in ("lod1", "cage", "proxy", "helper", "collision", "wire",
                                  "sensor", "lidar", "rig_pole", "calib", "occ")):
            o.hide_render = True
            o.hide_viewport = True

def force_hero_materials():
    """Cycle-12: hard-set cruiser paint / glass / wet road so Cycles reads AAA."""
    for mat in bpy.data.materials:
        bsdf = get_bsdf(mat)
        if not bsdf:
            continue
        n = mat.name.lower()
        if any(k in n for k in ("blue_metallic", "bodylivery", "hmpd_paint", "carpaint", "paintchip")) or n.endswith("paint"):
            # Deep navy clearcoat on the REAL body material (BodyLivery), not silver plate
            ...
            # Kill any image tex driving Base Color
            for link in list(mat.node_tree.links):
                if link.to_node == bsdf and link.to_socket.name == "Base Color":
                    mat.node_tree.links.remove(link)
        if n in ("steel",) or n.endswith("steel") or "cagesteel" in n or "chassis" in n:
            # Demote huge steel/cage areas so they don't read as silver car body
            if "cage" in n or "chassis" in n:
                bsdf.inputs["Base Color"].default_value = (0.04, 0.04, 0.045, 1)
                bsdf.inputs["Metallic"].default_value = 0.6
                bsdf.inputs["Roughness"].default_value = 0.45
            elif n == "steel":
                bsdf.inputs["Base Color"].default_value = (0.15, 0.16, 0.18, 1)
...
def make_wet_asphalt():
    ...
    mat = bpy.data.materials.new("WetAsphalt_C12")
    mat.use_nodes = True
    nt = mat.node_tree
    ...
    bsdf.inputs["Base Color"].default_value = (0.02, 0.021, 0.023, 1)
    bsdf.inputs["Metallic"].default_value = 0.0
    ...
    ramp.color_ramp.elements[0].color = (0.08, 0.08, 0.08, 1)  # wet patch
    ramp.color_ramp.elements[1].position = 0.78
    ramp.color_ramp.elements[1].color = (0.55, 0.55, 0.55, 1)  # damp/rough asphalt
    ...
    bpy.ops.object.transform_apply(scale=True)
    walk.name = "Sidewalk"
    wmat = bpy.data.materials.new("Sidewalk_C12")
    wmat.use_nodes = True
    wbsdf = get_bsdf(wmat)
...
def setup_world(night: bool):
    ...
    if night:
        bg.inputs["Color"].default_value = (0.02, 0.03, 0.055, 1)
...
def hide_poles_from_beauty():
    """Priority 6: remove distracting lamp poles / wires from beauty cams."""
    for o in list(bpy.data.objects):
        n = o.name.lower()
        if any(k in n for k in ("lamppole", "lamp_pole", "utilitypole", "wire", "cable", "powerline")):
            o.hide_render = True
            o.hide_viewport = True


def darken_architecture():
    """Priority 2: stop white-clay façades — force stone/paint/glass hierarchy."""
    for mat in bpy.data.materials:
        bsdf = get_bsdf(mat)
        if not bsdf:
            continue
        n = mat.name.lower()
        if any(k in n for k in ("glass", "window", "glaze", "windshield", "lens")):
            bsdf.inputs["Base Color"].default_value = (0.25, 0.38, 0.48, 1)
            ...
            continue
        if "brick" in n:
            bsdf.inputs["Base Color"].default_value = (0.30, 0.16, 0.12, 1)
            bsdf.inputs["Roughness"].default_value = 0.72
            continue
        if any(k in n for k in ("concrete", "stone", "limestone", "granite", "plaster",
                                  "stucco", "sidewalk", "curb", "asphalt")):
            # Warm stone / cool concrete hierarchy
            if "concrete" in n:
                bsdf.inputs["Base Color"].default_value = (0.28, 0.27, 0.25, 1)
                bsdf.inputs["Roughness"].default_value = 0.72
            elif "dark" in n:
                bsdf.inputs["Base Color"].default_value = (0.22, 0.21, 0.19, 1)
                bsdf.inputs["Roughness"].default_value = 0.65
            else:
                bsdf.inputs["Base Color"].default_value = (0.40, 0.37, 0.32, 1)
                bsdf.inputs["Roughness"].default_value = 0.58
            bsdf.inputs["Metallic"].default_value = 0.0
            continue
        if any(k in n for k in ("signwhite", "signface")):
            bsdf.inputs["Base Color"].default_value = (0.55, 0.52, 0.45, 1)
            bsdf.inputs["Roughness"].default_value = 0.5
            continue
        # Catch remaining near-white clay
        base = list(bsdf.inputs["Base Color"].default_value)
        if base[0] > 0.65 and base[1] > 0.65 and base[2] > 0.65 and "decal" not in n and "emit" not in n:
            bsdf.inputs["Base Color"].default_value = (0.42, 0.40, 0.37, 1)
            bsdf.inputs["Roughness"].default_value = max(0.5, float(bsdf.inputs["Roughness"].default_value))
...
def build_scene(night: bool):
    ...
    add_meridian_sign()
    add_facade_material_cards()
    add_reflection_env_balls()
    enhance_materials()
    force_hero_materials()
    darken_architecture()
    hide_junk()
    ...
    if night:
        # Practical window lights along Meridian Mutual
        for i, x in enumerate([30.0, 33.0, 36.0, 39.0, 42.0]):
            bpy.ops.object.light_add(type="AREA", location=fury_to_b(x, 2.8, 3.5 + (i % 3) * 2.5))
            Lw = bpy.context.active_object
            Lw.data.energy = 400
            Lw.data.size = 1.4
            Lw.data.color = (1.0, 0.85, 0.55)
            Lw.rotation_euler = (1.1, 0, 0)
        bpy.ops.object.light_add(type="POINT", location=fury_to_b(16.0, 3.2, 12.0))
        L = bpy.context.active_object
        L.data.energy = 600
        L.data.color = (1.0, 0.72, 0.42)
        L.data.shadow_soft_size = 0.35
        bpy.ops.object.light_add(type="POINT", location=fury_to_b(18.0, 1.6, 14.5))
        Lb = bpy.context.active_object
        Lb.data.energy = 280
        Lb.data.color = (0.25, 0.45, 1.0)
        # Soft fill so buildings aren't white clay against black void
        bpy.ops.object.light_add(type="AREA", location=fury_to_b(20.0, 8.0, -5.0))
        Lf = bpy.context.active_object
        Lf.data.energy = 120
        Lf.data.size = 18
        Lf.data.color = (0.35, 0.45, 0.75)
...
def main():
    ...
    print("DONE Cycle-12 Blender beauty →", OUT)
